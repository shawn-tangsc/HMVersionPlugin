...
@interface HMResourceManager()


@property (nonatomic, strong) HMJSVersionModel *jsVModel;
...
@implementation HMResourceManager



+ (instancetype)sharedInstance
{
    static HMResourceManager *_instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _instance = [[HMResourceManager alloc] init];
    });
    return _instance;
}
        ...
        if ([dict isKindOfClass:[NSDictionary class]]) {
            ...
            if (configData.length > 0) {
                ...
                if (writeSuccess) {
                    WXLogInfo(@"写入配置文件成功");
                    self.bmWidgetJs = nil;
                    [self showJsResourceUpdatedAlert];
...
-(void)checkDownloadZips:(NSString*)zipPath downloadPath:(NSString*)downloadPath
{
    ...
    __weak typeof(self) weakSelf = self;
    WXLogInfo(@"校验开始");
    [HMResourceCheck checkLocalResourceByZipPath:zipPath result:^(BOOL check, NSDictionary *info) {
        WXLogInfo(@"校验结束");
